...
function readLineNumber(fd, lineNumber) {
    ...
    while (true) {
        ...
        l = parseInt(line.toString().split(' ')[0], 10);
        if (l === lineNumber) {
            break;
        }
    }
...
function parseIndexLine(line, callback) {
    ...
    if (result.synsetCount === 1) {
        result.pos = 'n';
    } else if (result.synsetCount === 2) {
        result.pos = 'a';
    } else if (result.synsetCount === 4) {
        result.pos = 'r';
    } else if (result.synsetCount === 3) {
        result.pos = 'v';
    }
...
function parseDataLine(line, pointers, callback) {
    ...
    var synsetType;
    if (meta.lexFilenum === 1) {
        synsetType = 'n';
    } else if (meta.lexFilenum === 2) {
        synsetType = 'a';
    } else if (meta.lexFilenum === 4) {
        synsetType = 'r';
    } else if (meta.lexFilenum === 3) {
        synsetType = 'v';
    }

    meta.synsetType = _synsetTypeMap[synsetType];

    meta.wordCount = parseInt(metadata.shift(), 10);

    var wordsData = metadata.shift().split(':');
    meta.words = [];

    /* Parse the words */
    for (var wordIdx = 0; wordIdx < meta.wordCount; wordIdx++) {
        meta.words.push({
            word: wordsData[wordIdx]
...
function readData(definition, pointers, callback) {
    ...
    /* Read from file */
    var fd = _data[pos];
    var line = readLineNumber(fd, offset);
...
async.eachSeries(_extensions, readDatabase, function (err) {
    ...
    //     _data[extensionToPos[extension]] = fs.openSync(__dirname + '/../database/data.'+extension, 'r');
    // });
